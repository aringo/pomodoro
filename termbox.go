...
import (
	...
	"github.com/aringo/pomodoro/display"
	"github.com/nsf/termbox-go"
)

func fullscreenCountdown(start, finish time.Time, formatter func(time.Duration) string, task string) {
	err := termbox.Init()
	if err != nil {
...
func render(start, finish time.Time, formatter func(time.Duration) string) bool {
	...
	taskStr := "Current Task in Progress"
        display.Point{
		centerX - (len(taskStr) / 2), 0,
		termbox.ColorBlue, termbox.ColorDefault,
	}.Str(taskStr)
	display.Point{
		centerX - (len(*task) / 2), 1,
		termbox.ColorWhite, termbox.ColorDefault,
	}.Str(*task)
